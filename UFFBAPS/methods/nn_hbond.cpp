 ...
 */
/*
$Id: nn_hbond.cpp 6326 2010-08-26 16:08:21Z nielsen $
*/

#include "nn_hbond.h"


NN_HBOND::NN_HBOND(FILE * reporter):Nnmodel()
{

  /*** write out to out file **/
  writeDescription(reporter);
 
  printf("Using the NN_HBOND network!\n");
...
void NN_HBOND::build_inputs(pose * A)
{
  ...
  for(unsigned int p=0; p<protein_atoms.size(); p++)
	...
	if(dist < threshold)
	  ...
	  if(count(ignore_elements.begin(), ignore_elements.end(),protein_atoms[p]->element) < 1)
		...
		//temp end
		float invD1 = 1/dist;
		float invD2 = invD1*invD1;
		float invD6 = invD2*invD2*invD2;
		//float invD12 = invD6*invD6;

		//              res.push_back(invD1);
		//		res.push_back(invD6);
		//		res.push_back(invD12);
		res.push_back(dist);
...
void NN_HBOND::test_pose(pose * A)
{
  float energy = 0;
...
void NN_HBOND::train_pose(pose * A)
{
...
void NN_HBOND::write_network(string name)
{
...
void NN_HBOND::read_network(string name)
{
  /*** save network to file ***/
 for(int i=0; i<no_types; i++)  
    for(int j=0; j<no_types; j++)
...
void NN_HBOND::set_parameters()
{
  no_inputs =1;

  ifstream parameters;
  parameters.open("nn_hbond.cfg");

  if(! parameters.is_open())
    {
      cout << "Error:File 'nn_hbond.cfg' could not be found\n";
      exit(0);
    }
  ...
  printf("Configurations for vdw NN:\n");
  printf("\n");
  printf("\n");
  ...
  printf("Version: $Id: nn_hbond.cpp 6326 2010-08-26 16:08:21Z nielsen $\n");
  printf("\n");
...
int NN_HBOND::get_no_epochs(){return no_epochs;}
float NN_HBOND::get_target_factor(){return target_factor;}
int NN_HBOND::get_max_epochs_since_record(){return max_epochs_since_record;}
string NN_HBOND::get_dataset(){return dataset;}

float NN_HBOND::get_train_pct(){return train_pct;}
float NN_HBOND::get_validate_pct(){return validate_pct;}
float NN_HBOND::get_test_pct(){return test_pct;}

bool NN_HBOND::get_include_co_factors()
{
...
bool NN_HBOND::get_include_water()
{
  if(include_water == 1)
    return true;
  else
...
void NN_HBOND::writeDescription(FILE * reporter)
{
  /*** write out to out file **/
  version =string("$Id: nn_hbond.cpp 6326 2010-08-26 16:08:21Z nielsen $");
